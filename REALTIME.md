...
The `/api/dental-realtime/connect` endpoint accepts both the HTTP/1.1 WebSocket `GET` upgrade and the HTTP/2 extended `CONNECT` handshake. This avoids HTTP 405 responses when Chromium negotiates WebSockets over HTTP/2 on the HTTPS development URL.
